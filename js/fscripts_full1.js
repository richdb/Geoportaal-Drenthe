...
function flamingo_onInit() {
	...
	if (title.length > 0) {
		sTitle = "<FLAMINGO xmlns:fmc=\"fmc\"><fmc:Text id=\"titel_boven\" left=\"left\" top=\"top\"><style id=\".text\" font-family=\"arial,verdana\" font-size=\"24px\" color=\"#000000\" display=\"block\" font-weight=\"bold\"/><string id=\"text\"><nl><![CDATA[<span class=\"text\">";
		sTitle = sTitle + title;
		sTitle = sTitle + "</span>]]></nl></sting></fmc:Text></FLAMINGO>";
		app.call("flamingo","setConfig", sTitle);			
	}
		
	if (vis.length > 0) {
		sLegend = "<?xml version=\"1.0\" encoding=\"utf-8\"?><FLAMINGO xmlns:fmc=\"fmc\"><fmc:Legend id=\"legend\" skin=\"f2\" listento=\"map\" symbolpath=\"../../../output\" listento=\"map\" shadowsymbols=\"false\" width=\"215\" right=\"right\" top=\"top\" bottom=\"bottom\" backgroundcolor=\"#FFFFFF\"><group label=\"Kaartlaag\" open=\"true\">";
		sPlaat = gdb_legend.split(",");
		sLagen = vis.split(",");
		sTit = tvis.split("$");
		for (iRecord = 0; iRecord < sLagen.length; iRecord++) {			
			sLaag = "<item label=\"" + sTit[iRecord] + "\" canhide=\"true\" listento=\"geoportaal." + sLagen[iRecord] + "\"><symbol label=\"\" url=\"" + sPlaat[iRecord] + "\"/></item>";
			sLegend = sLegend + sLaag;
		}				
		app.call("flamingo","setConfig", sLegend + "</group></fmc:Legend></FLAMINGO>");	
	}
